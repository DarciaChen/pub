...
## [2026-08-03] 每次部署應寫入實際部署時間戳記，避免每次都靠猜快取或查API確認版本
情境：多次因 GitHub Pages CDN／行動裝置快取延遲，讓使用者反覆換 Wi-Fi、清快取、加網址亂數參數，才能確認畫面上是不是最新版本；每次都要我另外查 GitHub API 才能佐證「源頭是對的」。使用者提出更好的解法：往後每次改版都直接在頁面上寫一個部署時間戳記或版次編號，一打開就能對照。
教訓：對於會頻繁反覆部署、且使用者需要自行判斷「這是不是最新版」的前端工具，應在頁面顯眼處（不需捲動）加入部署時間戳記，並且**每次部署都必須更新這個戳記**（用部署當下的實際 UTC 時間，而非寫死或省略）。這樣使用者端可以自行判定是否命中快取，不需要每次都靠外部驗證或反覆換裝置測試。
制度修改：tools/prospect-leadgen-v3.html 已加入「部署版本：」戳記（commit a88d9e0d）。之後每次部署此檔案，應在推送前先用 `date -u +"%Y-%m-%d %H:%M UTC"` 產生當下時間並寫入對應欄位，不得省略此步驟。建議之後為 work-center 與 sales-visit-system 也比照加入。

## [2026-08-27] Gemini 2.5系列「思考」計入maxOutputTokens，導致設8192仍被截斷
情境：開發系統的JSON格式化步驟持續發生輸出被截斷的問題，一開始誤判為配額或稀釋問題，後來拿到一次「真的查到資料、但輸出寫到一半被切斷」的實際案例才查證出根因。
